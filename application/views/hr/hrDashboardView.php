...
if (!empty($recent_leaves)) {
  foreach ($recent_leaves as $l) {
    $total_req_count++;
    if (isset($l['seemrq_status']) && $l['seemrq_status'] == 'approved')
      $approved_count++;
  }
}
$reminder_count = 0;
if (!empty($recent_leaves)) {
  foreach ($recent_leaves as $l) {
    if (isset($l['seemrq_reminder']) && $l['seemrq_reminder'] == 1)
      $reminder_count++;
  }
}
?>
  ...
  <div class="main-content">
    ...
    <?php if ($reminder_count > 0): ?>
      <div class="reminder-banner">
        ...
        <div>
          ...
          <strong><?= $reminder_count ?></strong> leave request<?= $reminder_count > 1 ? 's' : '' ?> with pending
          reminders.
        </div>
    ...
    <div class="kpi-grid">
      ...
      <div class="kpi-card v-purple">
        ...
        <div class="kpi-track">
          <div class="kpi-track-fill" style="width:100%"></div>
        </div>
      </div>
      ...
      <div class="kpi-card v-green">
        ...
        <div class="kpi-track">
          <div class="kpi-track-fill" style="width:<?= $attendance_rate ?>%"></div>
        </div>
      </div>
      ...
      <div class="kpi-card v-amber">
        ...
        <div class="kpi-track">
          <div class="kpi-track-fill"
            style="width:<?= $total_staff > 0 ? round(($absent_today / $total_staff) * 100) : 0 ?>%"></div>
        </div>
      </div>
    ...
    <div class="dash-grid">
      ...
      <div class="panel">
        <div class="panel-head">
          <h6>
            <span class="ph-icon" style="--ph-bg:#d1fae5;--ph-color:#059669"><i
        ...
        <div class="att-donut-wrap">
          ...
          <div class="att-stat">
            <div class="att-row">
              <span class="ar-label"><i class="fas fa-circle me-1" style="color:#10b981;font-size:.55rem"></i>
                Present</span>
              <span class="ar-val" style="color:#059669"><?= $present_today ?></span>
            </div>
            <div class="att-row">
              <span class="ar-label"><i class="fas fa-circle me-1" style="color:#f87171;font-size:.55rem"></i>
                Absent</span>
      ...
      <div class="panel">
        ...
        <a href="<?= base_url('Employee/viewJobApplicants') ?>" class="btn-brand mt-3"
      ...
      <div class="panel" style="grid-column: span 2;">
        <div class="panel-head">
          <h6>
            <span class="ph-icon" style="--ph-bg:#fee2e2;--ph-color:#dc2626"><i
            ...
            <tbody>
              <?php if (!empty($recent_leaves)): ?>
                <?php foreach ($recent_leaves as $leave):
                  ...
                  ?>
                  <tr
                    class="<?= (isset($leave['seemrq_reminder']) && $leave['seemrq_reminder'] == 1) ? 'row-reminder' : '' ?>">
                    ...
                    <td data-label="Reason">
                      <div class="text-md-start text-end">
                        ...
                        <?php if (!empty($leave['seemrq_summary'])): ?>
                          <br><a href="javascript:void(0)" class="detail-link mt-1" onclick="showLeaveDetails(
                    ...
                    <td data-label="Leave Balance">
                      <div class="d-flex flex-column align-items-md-start align-items-end">
                        <div style="font-size:.78rem;color:var(--text-muted);margin-bottom:4px"><?= $taken ?> / 20 used
                        </div>
                        <div class="kpi-track m-0" style="width:90px">
                          <div class="kpi-track-fill"
                            style="width:<?= min(100, ($taken / 20) * 100) ?>%;background:<?= $bal_color ?>"></div>
                        </div>
                      </div>
                    </td>
      ...
      <div class="panel" style="grid-column: span 2;">
        <div class="panel-head">
          <h6>
            <span class="ph-icon" style="--ph-bg:#fef3c7;--ph-color:#d97706"><i class="fas fa-gift"></i></span>
            Upcoming Bonuses
          </h6>
          <a href="<?= base_url('Employee/BonusReportView') ?>">Full Report →</a>
        </div>

        <?php if (!empty($upcoming_bonuses)): ?>
          <div class="table-responsive">
            <table class="se-table table align-middle mb-0">
              <thead>
                <tr>
              ...
              <tbody>
                <?php foreach ($upcoming_bonuses as $bonus):
                  $dateObj = new DateTime($bonus->nearest_bonus_date);
                  $today = new DateTime();
                  $diff = (int) $today->diff($dateObj)->days;
                  if ($diff <= 7) {
                    $urgency_class = 'badge-inactive';
                    $urgency_label = 'This Week';
                    $days_color = 'var(--danger)';
                  } elseif ($diff <= 30) {
                    $urgency_class = 'badge-pending';
                    $urgency_label = 'This Month';
                    $days_color = 'var(--warning)';
                  } else {
                    $urgency_class = 'badge-active';
                    $urgency_label = 'Upcoming';
                    $days_color = 'var(--success)';
                  }
                  $initial = strtoupper(substr($bonus->seempd_name, 0, 1));
                  ?>
                  <tr>
                    <td data-label="Employee">
                      <div class="d-flex align-items-center gap-2">
                        <div class="bonus-avatar"><?= $initial ?></div>
                        <span class="fw-bold"
                          style="color:var(--text-primary)"><?= htmlspecialchars($bonus->seempd_name) ?></span>
                      </div>
                    </td>
                    <td data-label="Employee ID" style="color:var(--text-muted);font-size:.82rem">
                      <?= htmlspecialchars($bonus->seempd_empid) ?>
                    </td>
                    <td data-label="Bonus Date" style="font-weight:700;color:var(--brand-600)">
                      <i class="far fa-calendar-alt me-1" style="opacity:.6"></i>
                      <?= date('d M Y', strtotime($bonus->nearest_bonus_date)) ?>
                    </td>
                    <td data-label="Days Until">
                      <span style="font-size:1.1rem;font-weight:800;color:<?= $days_color ?>"><?= $diff ?></span>
                      <span style="font-size:.75rem;color:var(--text-muted);margin-left:3px">days</span>
                    </td>
        ...
        <?php else: ?>
          <div class="text-center py-4" style="color:var(--text-muted)">
            <i class="fas fa-gift fa-2x mb-2 d-block" style="opacity:.25"></i>
            No upcoming bonus dates found.
          </div>
        <?php endif; ?>
      </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content border-0 shadow-lg" style="border-radius:20px;overflow:hidden">
        <div class="modal-header"
          style="background:linear-gradient(135deg,var(--brand-600),var(--brand-500));border:none">
        ...
        <div class="modal-body" style="padding:24px">
          ...
          <div>
            ...
            <div id="modalSummary"
              style="background:var(--surface-2);border:1px solid var(--border);border-radius:var(--r-md);padding:14px;min-height:90px;line-height:1.65;color:var(--text-secondary);margin-top:8px;font-size:.875rem">
            </div>
          </div>
        </div>
