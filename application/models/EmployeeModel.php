...
class EmployeeModel extends CI_Model
{
    ...
    public function register_employee($employee, $details)
    {
        ...
        // 1. Insert into main employee table
        $this->db->insert('seemployee', $employee);

        // 2. Insert into employee details table
        $this->db->insert('seempdetails', $details);

        // 3. NEW: Record the initial 'Hired' state in the lifecycle ledger
        $history_data = [
            'empid' => $employee['seemp_id'],
            'status_action' => 'hired',
            'effective_date' => $details['seempd_joiningdate'],
            'reason' => 'New Registration'
        ];
        $this->db->insert('seemp_status_history', $history_data);

        // 4. Update applicant if exists
        if (!empty($details['seempd_jobaid'])) {
            $this->db->where('sejoba_id', $details['seempd_jobaid']);
            $this->db->update('sejobapplicant', ['sejoba_state' => 'selected']);
        }
