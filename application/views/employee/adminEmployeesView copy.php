              ...
              <tr>
                <td>
                  ...
                  <a href="javascript:void(0)" class="text-primary small text-decoration-none"
                    onclick="viewEmployeeQuickDetails('<?= $emp->seemp_id ?>', <?= json_encode($emp->seempd_name) ?>)">
        ...
        <div class="modal-body">
          ...
          <div class="card bg-light mb-4">
            <div class="card-body">
              ...
              <div id="inc_month_warning" class="alert alert-warning d-none" role="alert">
                <i class="fas fa-exclamation-triangle me-2"></i>
                <strong>Warning:</strong> An increment has already been applied this month for this employee.
                Submitting another will be rejected by the server.
              </div>
              <form action="<?= base_url('Employee/applyIncrement') ?>" method="POST">
                ...
                <div class="row g-3">
                  ...
                  <div class="col-md-3">
                    ...
                    <input type="number" step="0.01" class="form-control" name="inc_percentage" id="inc_percentage" oninput="calculateIncrement()" min='1' max="100" required>
                  </div>
                  <div class="col-md-3">
    ...
    function openIncrementModal(empid, name, currentSalary) {
      ...
      fetch('<?= base_url("Employee/getIncrementHistoryAjax/") ?>' + empid)
        ...
        .then(data => {
          ...
          // Check if any existing increment is in the same month as today
          const now = new Date();
          ...
          if (data.length === 0) {
            ...
            data.forEach(inc => {
              const incDate = new Date(inc.inc_effective_date);
              if (incDate.getFullYear() === now.getFullYear() && incDate.getMonth() === now.getMonth()) {
                hasThisMonth = true;
              }
              tbody.innerHTML += `<tr ${incDate.getFullYear() === now.getFullYear() && incDate.getMonth() === now.getMonth() ? 'class="table-warning"' : ''}>
          ...
          // Show a banner warning if this month already has an increment
          const warningEl = document.getElementById('inc_month_warning');
          if (hasThisMonth) {
